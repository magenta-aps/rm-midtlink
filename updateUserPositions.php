...
// Update user's email addresses (from all time), ONLY if it is changed

$sql = "SELECT u.name, u.uid, b.email FROM {users} u
        INNER JOIN {bsk_user_data} b ON b.user_id = u.name
        AND b.email != u.mail
        WHERE u.status = 1";  // AND b.updated >= ".(time()-108000);
$res = db_query($sql)->fetchAll();

foreach($res as $r) {
  $sql = "UPDATE {users} SET mail = :mail WHERE uid = :uid";
  db_query($sql,array(':mail'=>$r->email,':uid'=>$r->uid));
//  echo 'UPDATE '.$r->name.': '.$r->email."\n";
}
